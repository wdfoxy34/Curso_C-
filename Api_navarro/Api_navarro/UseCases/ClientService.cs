...
namespace Api_navarro.UseCases
{
    public class ClientUsecase
    {
        ...
        public ClientUsecase(IClientRepository repo)
        {
            _repo = repo;
        }
        public async Task<List<Client?>> BuscarAsync(string tipo, string valor)
        {
            ...
            {
                case "nome":
                    List<Client?> clients = await _repo.GetByNameAsync(valor);
                    return clients;
                case "cpf":
                    if (valor.Length != 11)
                        throw new ArgumentException("CPF invalido");
                    admin = await _repo.GetByCPFAsync(valor);
                    return admin is null ? new List<Client?>() : new List<Client?> { admin };
                case "telefone":
        ...
        public async Task<Client> AdicionarAsync(
            string nome,
            string email,
            string cpf,
            string telefone,
